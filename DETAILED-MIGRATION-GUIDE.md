...
- Approved rights to create Exchange service-principal pointers, Management Scopes, and role assignments; separate approved access is required for the manual Entra grant and AAP removals
...
$entraSp = Get-MgServicePrincipal -Filter "appId eq '$($app.AppId)'" -All |
...
Stop if no assignment is returned or the IDs do not match the reviewed inventory. After change approval, remove that one assignment manually:
...
Stop if the policy App ID or scope does not match the reviewed application. Remove that one policy manually:

```powershell
Remove-ApplicationAccessPolicy -Identity $policy.Identity -Confirm
```

Repeat the configuration, live mailbox, and normal workload tests, then capture a fresh inventory. Finish Cutover, observation, and Cleanup for this App ID before starting the next application's manual migration.

> Prepare supports an app with one policy-controlled mailbox permission. Script 03 stops before changes when the app has multiple controlled permissions because those roles require one coordinated migration design.
...
Broad Entra `Mail.Read` must be removed by a completed Cutover; if rollback restores it, live Graph access remains broad until a later Cutover succeeds. A passing configuration result does not replace live post-Cutover validation.
...
- Prepare every reviewed application separately; then finish manual Cutover, observation, and Cleanup for one App ID before starting another.
...
- [ ] Manual Cutover and rollback approved
- [ ] Live positive and negative tests passed
- [ ] Observation period completed
- [ ] Legacy AAP manually reviewed and removed
